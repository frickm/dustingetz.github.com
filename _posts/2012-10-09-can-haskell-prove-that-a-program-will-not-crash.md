...
Note that it is referential transparency (functions are expressions with no side effects), not the type system, that assures us the program will not crash. Types are a tool to prove that a program is referentially transparent and that all expressions are well-defined. While encoding potential for error in return types is really helpful to prevent NullPointerExceptions, you can still strive for referential transparency in any language, even with no type system (or an incomplete one like Java's).
